...
define method do-attach-medium
    (sheet :: <sheet>, medium :: <gtk2-medium>) => ()
...
define method destroy-medium
    (medium :: <gtk2-medium>, #next next-method) => ()
...
define method deallocate-medium
    (_port :: <gtk2-port>, medium :: <gtk2-medium>, #next next-method) => ()
  next-method();
...
define method invalidate-cached-drawing-state
    (medium :: <gtk2-medium>, new-state :: <integer>) => ()
...
define method medium-foreground-setter
    (fg :: <ink>, medium :: <gtk2-medium>, #next next-method) => (foreground :: <ink>)
  next-method();
...
define method medium-background-setter
    (bg :: <ink>, medium :: <gtk2-medium>, #next next-method) => (background :: <ink>)
  next-method();
...
define method update-drawing-state
    (medium :: <gtk2-medium>, #key font) => (drawable)
...
define method establish-brush
    (medium :: <gtk2-medium>, brush :: type-union(<standard-brush>, <ink>), drawable) => ()
...
define method establish-pen 
    (medium :: <gtk2-medium>, pen :: <standard-pen>, drawable) => ()
  //--- Update drawable state
end method establish-pen;

define method establish-font
    (medium :: <gtk2-medium>, font, drawable) => ()
  //--- Update drawable state
...
define method compute-clip-mask (medium :: <gtk2-medium>) => (mask)
  medium.%clip-mask 
  | (medium.%clip-mask
       := begin
...
define generic convert-ink-to-drawable-components
    (medium :: <gtk2-medium>, drawable, brush)
...
define method convert-ink-to-drawable-components
    (medium :: <gtk2-medium>, drawable, brush :: <stencil>)
 => (pixel, fill-style, operation, image :: false-or(<image>));
...
define method convert-ink-to-drawable-components
    (medium :: <gtk2-medium>, drawable, brush :: <pixmap>)
 => (pixel, fill-style, operation, image :: false-or(<image>));
...
define method convert-ink-to-drawable-components
    (medium :: <gtk2-medium>, drawable, brush :: <standard-brush>)
 => (pixel, fill-style, operation, image :: false-or(<image>));
...
define method draw-points
    (medium :: <gtk2-medium>, coord-seq :: <coordinate-sequence>) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  let drawable = update-drawing-state(medium);
  do-coordinates
    (method (x, y)
...
define method draw-line
    (medium :: <gtk2-medium>, x1, y1, x2, y2) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  let drawable = update-drawing-state(medium);
...
define method draw-rectangle
    (medium :: <gtk2-medium>, left, top, right, bottom,
     #key filled? = #t) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
...
define method draw-rectangles
    (medium :: <gtk2-medium>, coord-seq :: <coordinate-sequence>,
     #key filled? = #t) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  let drawable = update-drawing-state(medium);
  do-endpoint-coordinates
...
define method draw-ellipse
    (medium :: <gtk2-medium>, center-x, center-y, radius-1-dx,
...
define method draw-image
    (medium :: <gtk2-medium>, image :: <image>, x, y) => (record)
...
define method set-pixel
    (medium :: <gtk2-medium>, color :: <rgb-color>, x, y) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  let drawable  = medium-drawable(medium);
  let (r, g, b) = color-rgb(color);
...
define method set-pixels
    (medium :: <gtk2-medium>, color :: <rgb-color>, coord-seq :: <coordinate-sequence>)
 => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  let drawable  = medium-drawable(medium);
  let (r, g, b) = color-rgb(color);
...
define method start-path (medium :: <gtk2-medium>) => (record)
  //--- Start the path
  #f
end method start-path;

define method end-path (medium :: <gtk2-medium>) => (record)
  //--- End the path
  #f
end method end-path;

define method abort-path (medium :: <gtk2-medium>) => (record)
  //--- Abort the path
  #f
end method abort-path;

define method close-path (medium :: <gtk2-medium>) => (record)
  //--- Close the path
  #f
end method close-path;

define method stroke-path (medium :: <gtk2-medium>, #key filled?) => (record)
  let drawable = update-drawing-state(medium);
  //--- Stroke the path
  #f
end method stroke-path;

define method fill-path (medium :: <gtk2-medium>) => (record)
  let drawable = update-drawing-state(medium);
  //--- Fill the path
  #f
...
define method clip-from-path
    (medium :: <gtk2-medium>, #key function = $boole-and) => (record)
  //--- Clip from the path
  #f
end method clip-from-path;

define method save-clipping-region (medium :: <gtk2-medium>) => (record)
  //--- Push the clip region
  #f
end method save-clipping-region;

define method restore-clipping-region (medium :: <gtk2-medium>) => (record)
  //--- Pop the clip region
  #f
end method restore-clipping-region;

define method move-to (medium :: <gtk2-medium>, x, y) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  with-device-coordinates (transform, x, y)
    //--- Move-to
  end;
...
define method line-to (medium :: <gtk2-medium>, x, y) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
  with-device-coordinates (transform, x, y)
    //--- Line-to
  end;
...
define method arc-to (medium :: <gtk2-medium>, center-x, center-y,
...
define method curve-to (medium :: <gtk2-medium>, x1, y1, x2, y2, x3, y3) => (record)
  let transform = sheet-device-transform(medium-sheet(medium));
...
define method draw-pixmap
    (medium :: <gtk2-medium>, pixmap :: <pixmap>, x, y,
...
define method draw-text
    (medium :: <gtk2-medium>, string :: <string>, x, y,
...
define method draw-text
    (medium :: <gtk2-medium>, character :: <character>, x, y,
...
define method glyph-for-character
    (_port :: <gtk2-port>, char :: <character>, text-style :: <text-style>, #key font)
 => (index :: <integer>, font,
...
define method text-size
    (_port :: <gtk2-port>, char :: <character>,
...
define method font-metrics
    (text-style :: <text-style>, _port :: <gtk2-port>, #key character-set)
...
define method establish-clipping-region( medium :: <gtk2-medium> )
=> ()
