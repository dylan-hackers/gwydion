...
define method print-specializer (type :: <union>, stream :: <stream>)
  ...
  printing-logical-block (stream, prefix: "{", suffix: "}")
    ...
    unless (empty?(type.union-singletons))
      write(stream, "one-of(");
      for (object in type.union-singletons, first? = #t then #f)
        unless (first?)
          write(stream, ", ");
          pprint-newline(#"linear", stream);
        end;
    ...
    for(mtype in type.union-members,
        first? = type.union-singletons.empty? then #f)
        unless (first?)
          write(stream, ", ");
          pprint-newline(#"linear", stream);
        end;
...
define method print-object
  ...
  if (s & zero?(s))
    ...
    let f = r[0];
    let b   = if (~s | s > 1) r[1] - f else 1 end;
...
define method do-printing-object 
  ...
  printing-logical-block (stream, prefix: "{", suffix: "}")
    case
      type? & identity? =>
        write(stream, as-lowercase(as(<byte-string>, class.class-name)));
        write(stream, " ");
        continuation(stream);
        //--- write(stream, " ");
