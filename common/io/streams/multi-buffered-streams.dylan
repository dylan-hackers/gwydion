...
define constant <buffer-map> = <stretchy-vector>; // limited(<stretchy-vector>, of: <buffer-map-entry>);
...
define inline function buffer-map-entry-deposit-dirty
  ...
  if (v)
    ...
    logand(bme, lognot(ash(1, 1)))
  end
end function;

define inline function bit-field-extract
    (bit :: <integer>, count :: <integer>, value :: <integer>)
 => (res :: <integer>)
  let mask = ash(1, count) - 1;
  logand(mask, ash(value, - bit))
end function;

define inline function bit-field-deposit
    (field :: <integer>, bit :: <integer>, count :: <integer>,
     value :: <integer>)
 => (res :: <integer>)
  let mask = ash(1, count) - 1;
  logior(ash(logand(mask, field), bit),
         logand(value, lognot(ash(mask, bit))))
end function;
...
define inline function write-4-aligned-bytes-from-word
  ...
  with-output-buffer (sb = stream)
    ...
    let ptr = make(<machine-pointer>,
                   pointer: vector-elements-address(sb) + bi);
    signed-long-at(ptr) := as(<integer>, word);
    sb.buffer-next := bi + 4;
  ...
  with-input-buffer (sb = stream)
    if (sb)
      ...
      let ptr = make(<machine-pointer>,
                     pointer: vector-elements-address(sb) + bi);
      sb.buffer-next := bi + 4;
      as(<machine-word>, signed-long-at(ptr));
    else
